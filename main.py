...
async def feature_button(client, callback_query):
  ...
  keyboard = InlineKeyboardMarkup([
      [InlineKeyboardButton("📌 Auto Pin 📚 Course ", callback_data="pin_command")],
...
async def pin_button(client, callback_query):
  ...
  caption = f"**Auto Pin 📌 📚 Course  :**\n\nAutomatically Pins the 📚 Course  in Channel or Group, If Starting from the First Link."
  await callback_query.message.edit_media(
    InputMediaPhoto(
      media="https://tinypic.host/images/2025/07/14/file_000000002d44622f856a002a219cf27aconversation_id68747543-56d8-800e-ae47-bb6438a09851message_id8e8cbfb5-ea6c-4f59-974a-43bdf87130c0.png",
      caption=caption
...
async def txt_handler(bot: Client, m: Message):  
    ...
    await editable.edit(f"**Enter 📚 Course  or send /d**")
    try:
    ...
    try:
        for i in range(arg-1, len(links)):
            ...
            if "acecwply" in url:
                ...
                url = f"https://cpapi-ytas.onrender.com/extract_keys?url={url}@bots_updatee&user_id={7547625729}"
                ...
                signed_api = f"https://cpapi-ytas.onrender.com/extract_keys?url={url}@bots_updatee&user_id={7547625729}"
            ...
            try:
                if raw_text5 == "yes":
                    ...
                    ccimg = f'**Img Id :** {str(count).zfill(3)}\n**Img Title :** `{v_name}.jpg`\n\n<b>**Course Name :** {b_name}\n\n**Topic Name :** `{t_name}`'
                    cchtml = f'**Html Id :** {str(count).zfill(3)}\n**Html Title :** `{v_name}.html`\n\n<b>**Course Name :** {b_name}\n\n**Topic Name** : `{t_name}`'
                    ccyt = f'**Video Id :** {str(count).zfill(3)}\n**Video Title :** `{v_name}.mp4`\n\n<a href="{url}">__**Click Here to Watch Stream**__</a>\n**Course Name :** {b_name}\n\n**Topic Name :** `{t_name}`'
                    ccm = f'**Mp3 Id :** {str(count).zfill(3)}\n**Audio Title :** `{v_name}.mp3`\n\n<b>Course Name : {b_name}\nTopic Name : ⋅ ─ ✨ {t_name} ✨ ─ ⋅</b>\n'
                else:
                    cc = f'[🎞️] Vid Id : {str(count).zfill(3)}\n**Video Title :** `{name1} [{res}p]🌟𝐒нɑᎥ𝚝ɑη💖.mkv`\n\n<blockquote><b>📚 Course Name :</b> {b_name}</blockquote>\n\n**🌟 Download By : **{CR}\n'
                    cc1 = f'[📚]Pdf Id : {str(count).zfill(3)}\n**File Title :** `{name1}🌟𝐒нɑᎥ𝚝ɑη💖.pdf`\n\n<blockquote><b>📚 Course Name :</b> {b_name}</blockquote>\n\n**🌟 Download By : **{CR}\n'
                    cczip = f'[📁]Zip Id : {str(count).zfill(3)}\n**Zip Title :** `{name1}🌟𝐒нɑᎥ𝚝ɑη💖.zip`\n\n<blockquote><b>📚 Course Name :</b> {b_name}</blockquote>\n\n**🌟 Download By : **{CR}\n' 
                    ccimg = f'[🖼️]Img Id : {str(count).zfill(3)}\n**Img Title :** `{name1}🌟𝐒нɑᎥ𝚝ɑη💖.jpg`\n\n<blockquote><b>📚 Course Name :</b> {b_name}</blockquote>\n\n**🌟 Download By : **{CR}\n'
                    ccm = f'[🎵]Audio Id : {str(count).zfill(3)}\n**Audio Title :** `{name1}🌟𝐒нɑᎥ𝚝ɑη💖.mp3`\n\n<blockquote><b>📚 Course Name :</b> {b_name}</blockquote>\n\n**🌟 Download By : **{CR}\n'
                    cchtml = f'[🌐]Html Id : {str(count).zfill(3)}\n**Html Title :** `{name1}🌟𝐒нɑᎥ𝚝ɑη💖.html`\n\n<blockquote><b>📚 Course Name :</b> {b_name}</blockquote>\n\n**🌟 Download By : **{CR}\n'
...
async def text_handler(bot: Client, m: Message):
    ...
    try:
            ...
            if "acecwply" in url:
                ...
                url = url.replace("https://cpvod.testbook.com/","https://media-cdn.classplusapp.com/drm/")
                url = f"https://cpapi-rjbs.onrender.com/extract_keys?url={url}@bots_updatee&user_id={7547625729}"
                #url = f"https://scammer-keys.vercel.app/api?url={url}&token={cptoken}&auth=@scammer_botxz1"
                mpd, keys = helper.get_mps_and_keys(url)
                url = mpd
                keys_string = " ".join([f"--key {key}" for key in keys])
                ...
                signed_api = f"https://cpapi-rjbs.onrender.com/extract_keys?url={url}@bots_updatee&user_id={7547625729}"
                response = requests.get(signed_api, timeout=20)
                #url = response.text.strip()
                url = response.json()['url']  
